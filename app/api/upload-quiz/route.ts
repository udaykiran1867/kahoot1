...
const CORRECT_ANSWER_MAP: Record<string, number> = {
  A: 0,
  B: 1,
  C: 2,
  D: 3,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const quizTitle = (formData.get("title") as string) || "Imported Quiz"
    ...
    if (rows.length === 0) {
      return NextResponse.json(
        { success: false, error: "Excel sheet has no data rows" },
        { status: 400 }
      )
    }
    ...
    const skippedRows: Array<{ index: number; error: string }> = []
