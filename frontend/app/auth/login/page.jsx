...
const buttonDefault =
  `${buttonBase} bg-gradient-to-r from-violet-600 via-indigo-500 to-cyan-500 text-white shadow-lg shadow-indigo-500/25 hover:scale-[1.02] hover:shadow-cyan-400/35 active:scale-[0.99]`;
const inputClass =
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#2b1b80] via-[#2536a5] to-[#0f9ccf] p-4">
      <div className="pointer-events-none absolute inset-0 opacity-35" aria-hidden>
        <div className="absolute -top-24 -left-16 h-72 w-72 rounded-full bg-fuchsia-400/50 blur-3xl animate-pulse" />
        <div className="absolute top-24 -right-20 h-80 w-80 rounded-full bg-cyan-300/40 blur-3xl animate-pulse [animation-delay:700ms]" />
        <div className="absolute -bottom-20 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-indigo-300/45 blur-3xl animate-pulse [animation-delay:1200ms]" />
      </div>

      <div
        className={`relative z-10 w-full max-w-md rounded-3xl border border-white/30 bg-white/15 p-1 shadow-2xl backdrop-blur-xl transition-all duration-700 ease-out hover:-translate-y-0.5 ${
          entered ? "opacity-100 rotate-0 scale-100" : "opacity-0 -rotate-180 scale-90"
        }`}
      >
        <div className="rounded-[22px] bg-white/90 px-6 py-7 text-card-foreground">
          <div className="flex flex-col space-y-1.5 text-center">
            <Link href="/" className="group mb-4 flex items-center justify-center gap-2">
              <div className="rounded-xl bg-gradient-to-br from-violet-600 to-cyan-500 p-2 shadow-lg shadow-violet-400/30 transition-transform duration-300 group-hover:scale-105">
            ...
            <p className="text-sm text-slate-500">Let the quiz continue!</p>
          </div>
